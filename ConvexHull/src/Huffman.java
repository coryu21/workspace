public class Huffman {
	...
	public static void main(String args[]) {
		...
		int len[] = new int[27];
		int code[] = new int[27];
		int i, j, k, t, t1, t2, x, M = str.length(), cnt = 0;
		...
		System.out.println();
		for (i = 1; i <= huf.pq.n; i++){
			System.out.print(huf.pq.heap[i] + " ");

		}
		System.out.println();
		for (i = 1; i <= huf.pq.n; i++) {
			System.out.print((char) (huf.pq.info[i] + 64) + " ");
		}
		System.out.println();
		for (; !huf.isEmpty(); i++) {
			t1 = huf.remove_pq();
		...
		for (j = 0; j <= M; j++) {
			for (i = len[huf.index(text[j])]; i > 0; i--) {
				System.out.print(code[huf.index(text[j])] >> (i - 1) & 1);
				cnt++;
				if (cnt % 50 == 0)
					System.out.println();
			}
